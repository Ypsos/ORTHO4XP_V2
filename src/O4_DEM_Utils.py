...
try:
    import rasterio
    import rasterio.crs
    has_gdal = True   # rasterio remplace gdal — autonome dans venv
except:
    try:
...
def read_elevation_from_file(
    ...
    if file_name[-4:].lower() == ".hgt":
        ...
        try:
            # Utilise rasterio (autonome dans venv) ou gdal si disponible
            try:
                import rasterio
                with rasterio.open(file_name) as ds:
                    if not info_only:
                        alt_dem = ds.read(1).astype(numpy.float32)
                    (nydem, nxdem) = ds.shape
                    ...
                    if nodata_val is None:
                        ...
                        nodata = numpy.float32(nodata_val)
                        if not info_only:
