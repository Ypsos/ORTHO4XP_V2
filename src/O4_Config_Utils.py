...
class Tile:
    def __init__(self, lat, lon, custom_build_dir):
        # --- DEBUT CORRECTION ROLAND : STOP AU DESKTOP ---
        if custom_build_dir and "Desktop" in custom_build_dir:
            custom_build_dir = ""
...
class Ortho4XP_Config(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.frame_dem = tk.Frame(
            self.frame_cfg, border=0, padx=0, pady=self.pady, bg="#3b5b49"
        )
    ...
    def _open_simulator(self):
        try:
            ...
            lat = int(getattr(self.parent, 'lat', tk.StringVar()).get() or 46)
            lon = int(getattr(self.parent, 'lon', tk.StringVar()).get() or -3)
            custom = getattr(self.parent, 'custom_build_dir',
                tk.StringVar()).get() or ""
            if hasattr(self.parent, '_sim_win') and                     self.parent._sim_win and                     self.parent._sim_win.winfo_exists():
                self.parent._sim_win.lift()
                self.parent._sim_win.focus_force()
                return
            sim = _GUI.Ortho4XP_Simulator(self.parent, lat, lon, custom)
            self.parent._sim_win = sim
        except Exception as e:
            tk.messagebox.showinfo("Simulateur", f"Erreur : {e}")
    ...
    def load_tile_cfg(self):
        ...
        f.close()
        n = len(loaded)
        UI.vprint(1, "Tile config loaded from:", build_dir, "->", n, "variables")
############
    def write_tile_cfg(self):
        ...
        try:
            ...
            UI.vprint(1, "write_tile_cfg: cannot get lat/lon ->", e)
            return 0
        ...
        try:
            ...
            UI.lvprint(0, "Cannot write into", build_dir, "->", e)
        ...
        try:
            ...
            for var in list_tile_vars:
                ...
                try:
                    ...
                    value = v.get().strip() if hasattr(v, "get") else str(v).strip()
                    f.write(var + "=" + value + "\n")
                except Exception as e2:
                    UI.vprint(2, "write_tile_cfg: skipping var", var, "->", e2)

            # Ajouter default_website et default_zl si pas déjà écrits
            for var in ("default_website", "default_zl"):
                if var in list_tile_vars:
            ...
            f.close()
            UI.vprint(1, "Tile config written to:", config_file)
            return 1

        except Exception as e:
            UI.lvprint(0, "Error while writing tile cfg:", e)
            return 0
######
    def load_global_cfg(self):
        ...
        f.close()
        n = len(loaded)
        UI.vprint(1, "App config loaded from:", cfg_path, "->", n, "variables")
        return
    ...
    def write_global_cfg(self):
        ...
        try:
            ...
            self.popup("ERROR", "Could not write App config:\n" + str(e))
        return
